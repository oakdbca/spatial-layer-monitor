##################################################
# ADAPTED FROM OFFICIAL VERSION BY PATRICK MASLEN
# Adapted to enable being called from a Python script, e.g. 
# gdal_edit_args = ["", "-a_srs", "EPSG:28350", output_image]
...
def gdal_edit(args):
    ...
    while i < argc:
        if args[i] == '-ro':
            ro = True
        elif args[i] == '-a_srs' and i < len(args) - 1:
            srs = args[i + 1]
            i = i + 1
        elif args[i] == '-a_ullr' and i < len(args) - 4:
            ulx = float(args[i + 1])
            i = i + 1
            uly = float(args[i + 1])
            i = i + 1
            ...
            lry = float(args[i + 1])
            i = i + 1
        elif args[i] == '-a_ulurll' and i < len(args) - 6:
            ulx = float(args[i + 1])
            i = i + 1
            uly = float(args[i + 1])
            i = i + 1
            ...
            lly = float(args[i + 1])
            i = i + 1
        elif args[i] == '-tr' and i < len(args) - 2:
            xres = float(args[i + 1])
            i = i + 1
            yres = float(args[i + 1])
            i = i + 1
        elif args[i] == '-a_nodata' and i < len(args) - 1:
            nodata = float(args[i + 1])
            i = i + 1
        elif args[i] == '-scale' and i < len(args) :
            scale.append(float(args[i+1]))
            i = i + 1
            while i < len(args) - 1 and ArgIsNumeric(args[i+1]):
                scale.append(float(args[i+1]))
                i = i + 1
        elif args[i] == '-offset' and i < len(args) - 1:
            offset.append(float(args[i+1]))
            i = i + 1
            while i < len(args) - 1 and ArgIsNumeric(args[i+1]):
                offset.append(float(args[i+1]))
                i = i + 1
        elif args[i] == '-mo' and i < len(args) - 1:
            molist.append(args[i + 1])
            i = i + 1
        elif args[i] == '-gcp' and i + 4 < len(args):
            pixel = float(args[i + 1])
            i = i + 1
            line = float(args[i + 1])
            i = i + 1
            x = float(args[i + 1])
            ...
            y = float(args[i + 1])
            i = i + 1
            if i + 1 < len(args) and ArgIsNumeric(args[i + 1]):
                z = float(args[i + 1])
                i = i + 1
            else:
                z = 0
            ...
            unsetgt = True
        elif args[i] == '-unsetrpc':
            unsetrpc = True
        elif args[i] == '-unsetstats':
            unsetstats = True
        elif args[i] == '-approx_stats':
            stats = True
            approx_stats = True
        elif args[i] == '-stats':
            stats = True
        elif args[i] == '-setstats' and i < len(args)-4:
            stats = True
            setstats = True
            if args[i + 1] != 'None':
                statsmin = float(args[i + 1])
            else:
                statsmin = None
            i = i + 1
            if args[i + 1] != 'None':
                statsmax = float(args[i + 1])
            else:
                statsmax = None
            i = i + 1
            if args[i + 1] != 'None':
                statsmean = float(args[i + 1])
            else:
                statsmean = None
            i = i + 1
            if args[i + 1] != 'None':
                statsdev = float(args[i + 1])
            else:
                statsdev = None
            i = i + 1
        elif args[i] == '-units' and i < len(args) - 1:
            units = args[i + 1]
            i = i + 1
        elif args[i] == '-unsetmd':
            unsetmd = True
        elif args[i] == '-unsetnodata':
            unsetnodata = True
        elif args[i] == '-oo' and i < len(args) - 1:
            open_options.append(args[i + 1])
            i = i + 1
        elif args[i].startswith('-colorinterp_')and i < len(args) - 1:
            band = int(args[i][len('-colorinterp_'):])
            val = args[i + 1]
            if val.lower() == 'red':
...
def main(args):
    return gdal_edit(args)


if __name__ == '__main__':
    sys.exit(gdal_edit(args))
